...
import { GamepadEmulator, DEFAULT_GPAD_BUTTON_COUNT, DEFAULT_GPAD_AXIS_COUNT, type EGamepad, VariableButtonTouchConfig, ButtonTouchConfig } from "virtual-gamepad-lib/GamepadEmulator";
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    const addEmulatedGamepadButton = document.getElementById("add_emulated_gpad_btn") as HTMLElement;
    if (!gamepadDisplaySection || !gamepadDisplayTemplate || !addEmulatedGamepadButton) throw Error("gamepadDisplaySection or gamepadDisplayTemplate elements not found in DOM");
    ...
    function addEmulatedGamepad(overlay: boolean, index: number) {
        if (overlay) {
    ...
    function addGamepadDisplay(gpadIndex: number) {
        ...
        const joysticks: GamepadDisplayJoystick[] = [{
            joystickElement: display_gpad.querySelector(`#${PRESET_SVG_GPAD_BTN_IDS[standardGpadButtonMap.LStick]}`) as SVGElement,
            xAxisIndex: 0,
            yAxisIndex: 1,
            movementRange: 10,
        }, {
            joystickElement: display_gpad.querySelector(`#${PRESET_SVG_GPAD_BTN_IDS[standardGpadButtonMap.RStick]}`) as SVGElement,
            xAxisIndex: 2,
            yAxisIndex: 3,
            movementRange: 10,
        }]
        ...
        if (gpad.emulation === gamepadEmulationState.emulated) {
            ...
            (display_gpad.querySelector("#gpad-emulation-btn") as HTMLElement).innerHTML = "REMOVE Emulated Gamepad";
            ...
            (display_gpad.querySelector("#gpad-emulation-btn") as HTMLElement).innerHTML = "REMOVE Emulated Gamepad Overlay";
            ...
            (display_gpad.querySelector("#gpad-emulation-btn") as HTMLElement).innerHTML = "ADD Emulated Gamepad Overlay";
        }
    ...
    function handleGpadEmulationBtnClick(gpadIndex: number) {
