...
globalThis.onGameReady = () => {
    ...
    window.onkeydown = (e: KeyboardEvent) => {
        if (e.shiftKey || e.altKey || e.metaKey) return; // ignore key events with shift, alt, or meta keys pressed
        const key = e.key.toLowerCase();
        pressedKeyboardKeys.add(key);
        const numberKey = parseInt(key);
        const touchNotPress = e.ctrlKey;
        // []+- to move the left joystick
        if (key === "a") gpadEmulator.MoveAxis(EMULATED_GPAD_INDEX, standardGpadAxesMap.LStickX, -1);
        else if (key === "d") gpadEmulator.MoveAxis(EMULATED_GPAD_INDEX, standardGpadAxesMap.LStickX, 1);
        else if (key === "w") gpadEmulator.MoveAxis(EMULATED_GPAD_INDEX, standardGpadAxesMap.LStickY, -1);
        else if (key === "s") gpadEmulator.MoveAxis(EMULATED_GPAD_INDEX, standardGpadAxesMap.LStickY, 1);
        // arrow keys to move the right joystick (prevent default is to prevent scrolling)
        else if (key === "arrowleft") { gpadEmulator.MoveAxis(EMULATED_GPAD_INDEX, standardGpadAxesMap.RStickX, -1); e.preventDefault(); }
        else if (key === "arrowright") { gpadEmulator.MoveAxis(EMULATED_GPAD_INDEX, standardGpadAxesMap.RStickX, 1); e.preventDefault(); }
        else if (key === "arrowup") { gpadEmulator.MoveAxis(EMULATED_GPAD_INDEX, standardGpadAxesMap.RStickY, -1); e.preventDefault(); }
        else if (key === "arrowdown") { gpadEmulator.MoveAxis(EMULATED_GPAD_INDEX, standardGpadAxesMap.RStickY, 1); e.preventDefault(); }
        ...
        else if (key === " ") gpadEmulator.PressButton(EMULATED_GPAD_INDEX, standardGpadButtonMap.A, touchNotPress ? 0 : 1, true);
    ...
    window.onkeyup = (e: KeyboardEvent) => {
        ...
        // []+- to move the left joystick
        if (!pressedKeyboardKeys.has("a") && !pressedKeyboardKeys.has("d")) gpadEmulator.MoveAxis(EMULATED_GPAD_INDEX, standardGpadAxesMap.LStickX, 0);
        if (!pressedKeyboardKeys.has("w") && !pressedKeyboardKeys.has("s")) gpadEmulator.MoveAxis(EMULATED_GPAD_INDEX, standardGpadAxesMap.LStickY, 0);
        // arrow keys to move the right joystick
        if (!pressedKeyboardKeys.has("arrowleft") && !pressedKeyboardKeys.has("arrowright")) gpadEmulator.MoveAxis(EMULATED_GPAD_INDEX, standardGpadAxesMap.RStickX, 0);
        if (!pressedKeyboardKeys.has("arrowup") && !pressedKeyboardKeys.has("arrowdown")) gpadEmulator.MoveAxis(EMULATED_GPAD_INDEX, standardGpadAxesMap.RStickY, 0);
        // gamepad buttons 0-9 are mapped to the number keys
        if (!isNaN(numberKey)) gpadEmulator.PressButton(EMULATED_GPAD_INDEX, numberKey, 0, false);
        else if (e.key === " ") gpadEmulator.PressButton(EMULATED_GPAD_INDEX, standardGpadButtonMap.A, 0, false);
    };
}
