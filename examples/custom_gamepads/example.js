...
const onPageLoad = () => {
    ...
    function setupEmulatedGamepadInput(gpadIndex) {
        /* ----- SETUP BUTTON INPUTS ----- */
    ...
    function setupGamepadDisplay(gpadIndex) {
        ...
        const joysticks = [{
                joystickElement: SPLIT_GPAD_DISPLAY_CONTAINER.querySelector(`#${PRESET_SVG_GPAD_BTN_IDS[standardGpadButtonMap.LStick]}`),
                xAxisIndex: 0,
                yAxisIndex: 1,
                movementRange: 10,
                extraData: {
                    directionHighlightArrows: {
                        [gamepadDirection.up]: SPLIT_GPAD_DISPLAY_CONTAINER.querySelector("#l_stick_up_direction_highlight"),
                        [gamepadDirection.down]: SPLIT_GPAD_DISPLAY_CONTAINER.querySelector("#l_stick_down_direction_highlight"),
                        [gamepadDirection.left]: SPLIT_GPAD_DISPLAY_CONTAINER.querySelector("#l_stick_left_direction_highlight"),
                        [gamepadDirection.right]: SPLIT_GPAD_DISPLAY_CONTAINER.querySelector("#l_stick_right_direction_highlight"),
                    }
                }
            }, {
                joystickElement: SPLIT_GPAD_DISPLAY_CONTAINER.querySelector(`#${PRESET_SVG_GPAD_BTN_IDS[standardGpadButtonMap.RStick]}`),
                xAxisIndex: 2,
                yAxisIndex: 3,
                movementRange: 10,
                extraData: {
                    directionHighlightArrows: {
                        [gamepadDirection.up]: SPLIT_GPAD_DISPLAY_CONTAINER.querySelector("#r_stick_up_direction_highlight"),
                        [gamepadDirection.down]: SPLIT_GPAD_DISPLAY_CONTAINER.querySelector("#r_stick_down_direction_highlight"),
                        [gamepadDirection.left]: SPLIT_GPAD_DISPLAY_CONTAINER.querySelector("#r_stick_left_direction_highlight"),
                        [gamepadDirection.right]: SPLIT_GPAD_DISPLAY_CONTAINER.querySelector("#r_stick_right_direction_highlight"),
                    }
                }
            }];
        ...
        const display = new GamepadDisplay({
            ...
            buttonDisplayFunction: function (buttonConfig, value, touched, pressed, changes, btnIndex) {
    ...
    /** Setup the touch targets & input parameters for translating onscreen events into events for the emulated gamepad (part of the emulated gamepad module) */
    function setupCustomEmulatedGamepadInput(gpadIndex) {
        /* ----- SETUP BUTTON INPUTS ----- */
        const emulatorButtonConfigs = [];
    ...
    /** Setup the display buttons & axes of the onscreen gamepad to react to the state of the gamepad from the browser gamepad api (uses the gamepadApiWrapper) */
    function setupCustomGamepadDisplay(gpadIndex) {
        ...
        for (let i = 0; i <= 2; i++) {
            let direction;
            if (i == 0)
                direction = gamepadDirection.left; // button 0 is setup to be dragged left only
            else if (i == 1)
                direction = gamepadDirection.down; // button 1 is setup to be dragged up only
            else
                direction = gamepadDirection.right; // button 2 is setup to be dragged down only
            displayButtons.push({
                type: gamepadButtonType.variable,
                direction: direction,
                ...
                highlight: CUSTOM_HTML_GPAD_DISPLAY_CONTAINER.querySelector("#drag-button-" + i),
            });
        }
        // Add Normal Buttons (buttons labeled/indexed 3-20)
        for (let i = 3; i <= 20; i++) {
        ...
        displayButtons.push({
            type: gamepadButtonType.onOff,
            highlight: CUSTOM_HTML_GPAD_DISPLAY_CONTAINER.querySelector(`#axis-0 .gpad-custom-thumbstick`),
        });
        /* ----- MAKE JOYSTICK DISPLAY CONFIG ARRAY ----- */
        const joysticks = [{
                joystickElement: CUSTOM_HTML_GPAD_DISPLAY_CONTAINER.querySelector(`#axis-0 .gpad-custom-thumbstick`),
                xAxisIndex: 0,
                movementRange: 100 - 30,
            }, {
                joystickElement: CUSTOM_HTML_GPAD_DISPLAY_CONTAINER.querySelector(`#axis-1 .gpad-custom-thumbstick`),
                yAxisIndex: 1,
                movementRange: 200 - 60,
            }, {
                joystickElement: CUSTOM_HTML_GPAD_DISPLAY_CONTAINER.querySelector(`#axis-2 .gpad-custom-thumbstick`),
                yAxisIndex: 3,
                movementRange: 200 - 60,
            }, {
                joystickElement: CUSTOM_HTML_GPAD_DISPLAY_CONTAINER.querySelector(`#axis-3 .gpad-custom-thumbstick`),
                xAxisIndex: 2,
                movementRange: 100 - 30,
            }, {
                joystickElement: CUSTOM_HTML_GPAD_DISPLAY_CONTAINER.querySelector(`#axis-4 .gpad-custom-thumbstick`),
                xAxisIndex: 4,
                yAxisIndex: 5,
                movementRange: 100 - 30,
            }];
...
if (document.readyState === "complete")
    ...
    window.addEventListener("load", onPageLoad); // otherwise, wait for the page to load before calling onPageLoad.
