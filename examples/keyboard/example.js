...
if (!eventDisplayElem)
    throw new Error("Could not find element with id 'gamepad-event-display'");
if (!buttonTable)
    throw new Error("Could not find element with id 'button-table'");
if (!gamepadEventArea)
    throw new Error("Could not find element with id 'gamepad-event-area'");
...
gpadApiWrapper.onGamepadButtonChange((gpadIndex, gpad, buttonChangesMask) => {
    ...
    for (let i = 0; i < gpad.buttons.length; i++) {
        ...
        if (!change)
            continue; // this button did not change
        ...
        if (btnTableRow)
            btnTableRow.children[1].style.backgroundColor = gpad.buttons[i].pressed ? "blueviolet" : (gpad.buttons[i].touched ? "greenyellow" : "");
    }
...
gpadApiWrapper.onGamepadAxisChange((gpadIndex, gpad, axisChangesMask) => {
    ...
    for (let i = 0; i < axisChangesMask.length; i++) {
        ...
        if (!change)
            continue; // this axis did not change
...
window.onkeydown = (e) => {
    ...
    // all other gamepad buttons are mapped to the number keys or keycodes for high button numbers
        ...
        gamepadEmu.PressButton(EMULATED_GPAD_INDEX, numberKey, touchNotPress ? 0 : 1, true);
    else if (e.keyCode)
        gamepadEmu.PressButton(EMULATED_GPAD_INDEX, e.keyCode - 65 + 10, touchNotPress ? 0 : 1, true); // 65 is the keycode for "a", 10 is the count of number keys on the keyboard (0-9), so "a" is button #10, "b" is button #11, etc.
};
window.onkeyup = (e) => {
    const numberKey = parseInt(e.key);
    // <>l/ to move the left stick
    ...
    // all other gamepad buttons are mapped to the number keys or keycodes for high button numbers
        ...
        gamepadEmu.PressButton(EMULATED_GPAD_INDEX, e.keyCode - 65 + 10, 0, false); // 65 is the keycode for "a", 10 is the count of number keys on the keyboard (0-9), so "a" is button #10, "b" is button #11, etc.
};
