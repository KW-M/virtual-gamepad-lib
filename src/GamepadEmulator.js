...
export const DEFAULT_GPAD_BUTTON_COUNT = 18;
/** A number of typical axes recognized by Gamepad API and mapped to
 * standard controls. Any extraneous axes will have larger indexes.*/
export const DEFAULT_GPAD_AXIS_COUNT = 4;
...
export class GamepadEmulator {
    ...
    AddEmulatedGamepad(gpadIndex, overlayMode, buttonCount = DEFAULT_GPAD_BUTTON_COUNT, axisCount = DEFAULT_GPAD_AXIS_COUNT) {
        ...
        const eGpad = {
            emulation: "emulated" /* gamepadEmulationState.emulated */,
            connected: true,
    ...
    */
    PressButton(gpadIndex, buttonIndex, value, touched) {
        if (this.emulatedGamepads[gpadIndex] === undefined)
    ...
    * @param buttonConfigs - an array of config objects that set how each of the buttons on the onscreen gamepad should behave, and how they map to the emulated gamepad buttons. */
    AddDisplayButtonEventListeners(gpadIndex, buttonConfigs) {
        ...
        for (var i = 0; i < buttonConfigs.length; i++) {
            ...
            if (btnConfig.type == "onOff" /* gamepadButtonType.onOff */) {
            ...
            else if (btnConfig.type == "variable" /* gamepadButtonType.variable */) {
                ...
                const removeDragListeners = this.AddDragControlListener(config, (pointerDown, xValue, yValue) => {
                    ...
                    value += (btnConfig.directions["left" /* gamepadDirection.left */] || btnConfig.directions["right" /* gamepadDirection.right */]) ? Math.abs(xValue) : 0;
                    value += (btnConfig.directions["up" /* gamepadDirection.up */] || btnConfig.directions["down" /* gamepadDirection.down */]) ? Math.abs(yValue) : 0;
    ...
    * @param gpadIndex - the index of the emulated gamepad to un-register events for */
    ...
    AddDragControlListener(config, callback) {
        ...
        const pointerMoveHandler = (moveEvent) => {
            ...
            if (activePointerId === pointerId) {
                const xMin = config.directions["left" /* gamepadDirection.left */] ? -1 : 0;
                const xMax = config.directions["right" /* gamepadDirection.right */] ? 1 : 0;
                const yMin = config.directions["up" /* gamepadDirection.up */] ? -1 : 0;
                const yMax = config.directions["down" /* gamepadDirection.down */] ? 1 : 0;
    ...
    cloneGamepad(original) {
        ...
        // @ts-ignore
        const clone = {}; //Object.create(Gamepad.prototype)
        for (let key in original) {
            if (key === "axes") {
                ...
                Object.defineProperty(clone, key, { get: () => { return original[key]; }, configurable: true, enumerable: true });
            }
        }
        // add extra emulation properties onto the gamepad
        if (!clone.emulation)
            clone.emulation = "real" /* gamepadEmulationState.real */;
        return clone;
    }
    ...
    * this means no emulated gamepad is at that index and either the no real gamepad is at that index, or a real gamepad is at that index, but the @param overlayMode is true. */
    nextEmptyEGpadIndex(overlayMode) {
        ...
        if (overlayMode) {
            ...
            do {
                if (!this.emulatedGamepads[index] && this.patchedGpadToRealIndexMap[index] == undefined)
                    break;
    ...
    monkeyPatchGamepadEvents() {
        ...
        const gamepadConnectedHandler = (e) => {
            ...
            if (gpad && gpad.emulation === undefined) {
                ...
                Object.defineProperty(eGpad, "emulation", { get: () => "real" /* gamepadEmulationState.real */ });
                ...
                if (windowOngamepadconnected)
                    windowOngamepadconnected.call(window, event);
            }
        };
        ...
        const gamepadDisconnectedHandler = (e) => {
            ...
            if (raw_gpad && raw_gpad.emulation === undefined) {
                e.stopImmediatePropagation(); // prevent future event listeners from firing
                ...
                if (windowOngamepaddisconnected)
                    windowOngamepaddisconnected.call(window, event);
            }
        };
        ...
        navigator.getGamepads = function () {
            ...
            for (let i = 0; i < emulatedGpads.length; i++) {
                ...
                if (e_gpad && n_gpad) {
                    ...
                    n_gpad.emulation = "overlay" /* gamepadEmulationState.overlay */;
                    ...
                    e_gpad.emulation = "emulated" /* gamepadEmulationState.emulated */;
